  ...
  useEffect(() => {
    const loadLeaguesAndSettings = async () => {
      try {
        ...
        // Load system settings (don't block league loading if this fails)
        try {
          ...
          console.error('Error loading league settings:', settingsError);
        }
        ...
        const leaguesData = leaguesSnapshot.docs.map(doc => ({
          ...
          archivedAt: normalizeDate(doc.data().archivedAt)
        }));
