...
import { collection, query, getDocs, doc, updateDoc, deleteDoc, where, arrayRemove } from 'firebase/firestore';
...
const ManageUsers = () => {
  ...
  const deleteUserAccount = async (userId) => {
    ...
    const confirmDelete = window.confirm(
      'Delete this user record? This will remove them from league rosters but will not delete their Auth account.'
    );
    ...
    try {
      ...
      const updatePromises = leaguesSnapshot.docs.map((leagueSnap) => {
        const leagueRef = doc(db, 'leagues', leagueSnap.id);
        const leagueData = leagueSnap.data() || {};
        const updatePayload = {
          members: arrayRemove(userId)
        };
        if (Array.isArray(leagueData.userIds)) {
          updatePayload.userIds = arrayRemove(userId);
        }
        if (Array.isArray(leagueData.users)) {
          updatePayload.users = leagueData.users.filter((user) =>
            typeof user === 'string' ? user !== userId : user?.id !== userId
          );
        }
        return updateDoc(leagueRef, updatePayload);
      });
