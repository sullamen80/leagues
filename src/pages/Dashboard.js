...
const Dashboard = () => {
 ...
 // Only set active league if at root path
 if (leagues.active.length > 0 && location.pathname === '/') {
 ...
 navigate(`/league/${activeLeague}/view?userId=${bracketId}`);
 };
 ...
 {activeLeague && currentLeagueData && location.pathname === '/' && (
 ...
 isEmbedded={true} // Indicate embedded context
 onViewBracket={handleViewBracketFromLeaderboard} // Pass custom navigation handler
 />
 </div>
 )}
 {currentLeagueData.gameTypeId !== 'marchMadness' && (
 <div className="text-center py-2 sm:py-4 md:py-8">
 <p className="text-gray-300 text-sm">Visit the full league page to interact with this league type.</p>
 <button onClick={() => navigate(`/league/${activeLeague}`)} className="mt-3 sm:mt-4 px-3 py-1.5 sm:px-4 sm:py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition text-sm">
 Open League
 </button>
 </div>
 )}
 ...
 <div key={gameType.id} className="bg-gray-800 border border-gray-700 p-2 sm:p-3 md:p-4 rounded-lg hover:bg brutality-700 transition">
