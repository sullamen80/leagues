...
const BaseEditor = ({
  ...
  const { entryId: customEntryId, ...restEditorProps } = editorProps || {};
  const resolvedEntryId = customEntryId ?? userId;
  ...
  const handleReset = () => {
    ...
    if (confirmReset) {
      // Pass both gameData AND userEntry to the resetEntry function to preserve Play-In data
      const resetData = resetEntry(gameData, userEntry);
