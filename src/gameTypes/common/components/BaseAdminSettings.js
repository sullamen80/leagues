 ...
 * @param {Function} props.onBack - Custom back handler function
 */
function BaseAdminSettings({
  ...
  useEffect(() => {
    ...
    // We intentionally omit activeTab from deps so the effect only runs when defaultTab updates.
  ...
  // Navigate back to admin dashboard - updated to use query parameters
  const handleBack = () => {
    if (onBack) {
  ...
  const updateTabInUrl = (tabId) => {
    const searchParams = new URLSearchParams(location.search);
    if (tabId) {
      searchParams.set('settingsTab', tabId);
    } else {
      searchParams.delete('settingsTab');
    }
    const queryString = searchParams.toString();
    const target = queryString ? `${location.pathname}?${queryString}` : location.pathname;
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-6">
        {tabs.map((tab) => {
          ...
          let content = null;
          if (React.isValidElement(tab.panel)) {
            content = React.cloneElement(tab.panel, panelProps);
          } else if (typeof tab.panel === 'function') {
            content = tab.panel(panelProps);
          } else {
            content = tab.panel || null;
          }
