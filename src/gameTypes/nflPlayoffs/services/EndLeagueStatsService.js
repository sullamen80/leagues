// src/gameTypes/nflPlayoffs/services/EndLeagueStatsService.js
import BaseEndLeagueStats from '../../common/services/BaseEndLeagueStatsService';
import { doc, getDoc, collection, getDocs, setDoc } from 'firebase/firestore';
import { db } from '../../../firebase';
import { calculateUserScore } from './scoringService';
import { ROUND_KEYS } from '../constants/playoffConstants';

/**
 * NFL Playoffs specific implementation of EndLeagueStats
 */
class EndLeagueStats extends BaseEndLeagueStats {
  /**
   * Initialize the NFL Playoffs stats collector
   * @param {string} leagueId - The league ID
   */
  constructor(leagueId) {
    super(leagueId, "nflPlayoffs");
  }
      ...
      for (const bracketDoc of userBracketsSnap.docs) {
        try {
          ...
          scores.push({
            ...
            championPick: bracketData[ROUND_KEYS.SUPER_BOWL]?.winner || "None Selected",
            championCorrect: bracketData[ROUND_KEYS.SUPER_BOWL]?.winner === this.gameData[ROUND_KEYS.SUPER_BOWL]?.winner,
   ...
   * @returns {Object} - NFL Playoffs specific stats
   */
  ...
  /**
   * Prepare and store the league stats with NFL Playoffs specific data
  ...
  async prepareAndStoreStats(winners) {
    ...
    const statsData = {
      ...
      // NFL Playoffs specific stats
      nflPlayoffsStats: {
        champion: this.gameData[ROUND_KEYS.CHAMPION] || this.gameData[ROUND_KEYS.SUPER_BOWL]?.winner || null,
  ...
  isLeagueCompleted() {
    return !!(this.gameData[ROUND_KEYS.CHAMPION] || 
             (this.gameData[ROUND_KEYS.SUPER_BOWL]?.winner && 
  ...
  getTotalPossiblePicks() {
    ...
    // Super Bowl
    if (this.gameData[ROUND_KEYS.SUPER_BOWL] && this.gameData[ROUND_KEYS.SUPER_BOWL].winner) {
      total += 1;
    }
  ...
  async generateStatsPreview(gameData) {
    try {
      ...
      // This is NFL Playoffs specific preview data
      ...
      return {
        ...
        // Other NFL Playoffs specific preview data
