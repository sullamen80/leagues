// src/gameTypes/nflPlayoffs/components/AdminStats.js
...
/**
 * Enhanced component for managing NFL Playoffs statistics with preview functionality
...
const AdminStats = ({ leagueId: propLeagueId, gameData: propGameData }) => {
  ...
  const getRoundPossiblePicks = (roundKey) => {
    if (!gameData) return 0;
    if (roundKey === ROUND_KEYS.SUPER_BOWL) {
      return gameData[roundKey]?.winner ? 1 : 0;
    }
    if (roundKey === ROUND_KEYS.FINALS_MVP) {
      return gameData[ROUND_KEYS.FINALS_MVP] ? 1 : 0;
    }
    if (roundKey === SUPER_WINNER_KEY) {
      return (gameData[ROUND_KEYS.CHAMPION] || gameData[ROUND_KEYS.SUPER_BOWL]?.winner) ? 1 : 0;
    }
    if (roundKey === PROP_BETS_KEY) {
      return 0;
    }
    if (Array.isArray(gameData[roundKey])) {
      return gameData[roundKey].filter((match) => match && match.winner).length;
    }
    return 0;
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (currentGameData) {
          try {
            const { rankings } = await calculateLeagueScores(leagueId, scoringSettings);
            setUserScores(rankings || []);
          } catch (scoresError) {
  ...
  const getTournamentStatus = () => {
    ...
    if (gameData[ROUND_KEYS.CHAMPION] || gameData?.[ROUND_KEYS.SUPER_BOWL]?.winner) return 'Completed';
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">NFL Playoffs Stats Manager</h2>
        <button 
        ...
        <div className="flex items-start mb-4">
          ...
          <div>
            <p className="font-medium text-gray-700">
              NFL Playoffs Statistics Management
            </p>
            <p className="text-sm text-gray-600">
      ...
      {!showPreview && (
        <div className="text-sm bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p><strong>Champion:</strong> {gameData?.[ROUND_KEYS.CHAMPION] || gameData?.[ROUND_KEYS.SUPER_BOWL]?.winner || 'Not determined'}</p>
                          ...
                          <div key={`${player.userId || playerIndex}-breakdown`} className="border rounded-lg p-3">
                            ...
                            <div className="space-y-2">
                              {player.roundBreakdown && Object.entries(player.roundBreakdown).map(([round, data]) => {
                                const roundPossiblePicks = getRoundPossiblePicks(round);
                                const roundCorrectPicks = round === SUPER_WINNER_KEY
                                  ? data.correctPrediction ? 1 : 0
                                  : data.correctPicks || 0;
                                const roundPercentage = roundPossiblePicks > 0 ? roundCorrectPicks / roundPossiblePicks : 0;
                                ...
                                return (
                                  <div key={round} className="bg-gray-50 p-2 rounded">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs">
                                      ...
                                      <p><span className="text-gray-600">Correct Picks:</span> {roundCorrectPicks}</p>
