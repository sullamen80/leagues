...
import { FaArrowLeft, FaUser, FaUserMinus, FaUserCheck, FaUserTimes, FaSort, FaSortUp, FaSortDown, FaCalendarAlt, FaClipboardCheck, FaTimesCircle, FaExclamationTriangle, FaArchive } from 'react-icons/fa';

/**
 * Admin component for managing league participants
 */
const AdminTeams = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log(`Found ${allMembers.length} total league members`);
        
        // Fetch all bracket data from userData collection
        const userBracketsRef = collection(db, "leagues", leagueId, "userData");
        const userBracketsSnap = await getDocs(userBracketsRef);
        
        // Map of userId -> bracket data
        const bracketDataMap = {};
        userBracketsSnap.forEach(doc => {
          bracketDataMap[doc.id] = doc.data();
        });
        
        console.log(`Found ${Object.keys(bracketDataMap).length} users with bracket data`);
        
        // Include users without bracket data but in members array
        allMembers.forEach(userId => {
          if (!bracketDataMap[userId]) {
            bracketDataMap[userId] = { hasEmptyBracket: true };
          }
        });
        
        // Get user data for each member
        const participantPromises = Object.keys(bracketDataMap).map(async (userId) => {
          const bracketData = bracketDataMap[userId];
          
          // Skip current user (admin) but include in the list marked as admin
          if (userId === currentUserId) {
            const adminData = { 
              id: userId, 
              bracketData: bracketData.hasEmptyBracket ? {} : bracketData,
              isAdmin: true,
              name: 'You (Admin)',
              email: auth.currentUser?.email || 'Unknown',
              joinedAt: bracketData.joinedAt || league.createdAt,
              hasBracket: !bracketData.hasEmptyBracket && !!bracketData.RoundOf64 && 
                bracketData.RoundOf64.some(m => m && m.winner)
            };
            return adminData;
          }
          ...
          let userData = {
            ...
            email: 'Unknown',
            isAdmin: false,
            joinedAt: bracketData.joinedAt || null,
            hasBracket: !bracketData.hasEmptyBracket && !!bracketData.RoundOf64 && 
              bracketData.RoundOf64.some(m => m && m.winner)
          };
          ...
          try {
            ...
            if (userSnap.exists()) {
              ...
              userData.name = userDoc.displayName || userDoc.username || userDoc.email || 'Unknown User';
              userData.email = userDoc.email || 'Unknown';
            }
          } catch (userError) {
  ...
  const sortData = (data, key, direction) => {
    return [...data].sort((a, b) => {
      ...
      // Handle boolean values (hasBracket)
  ...
  const handleSort = (key) => {
    ...
    const sortedData = sortData(participants, key, direction);
  ...
  const confirmRemoveParticipant = async () => {
    ...
    try {
      ...
      // 1. Delete the user's bracket from the league (if it exists)
      try {
        ...
        if (bracketSnap.exists()) {
          ...
          console.log("Deleted user bracket data");
        }
      } catch (err) {
        console.error("Error deleting bracket data:", err);
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th 
                className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                onClick={() => handleSort('hasBracket')}
              >
                <div className="flex items-center">
                  <span>Bracket Status</span>
                  {sortConfig.key === 'hasBracket' && (
                    <span className="ml-1">
                      {sortConfig.direction === 'asc' ? <FaSortUp /> : <FaSortDown />}
                    </span>
                  )}
                  {sortConfig.key !== 'hasBracket' && <FaSort className="ml-1 text-gray-300" />}
                </div>
              </th>
            ...
            {filteredParticipants.length === 0 ? (
              ...
              filteredParticipants.map(participant => (
                <tr key={participant.id} className={participant.isAdmin ? 'bg-blue-50' : ''}>
                  ...
                  <td className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap">
                    {participant.hasBracket ? (
                      <div className="flex items-center">
                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                          <FaClipboardCheck className="mr-1" /> Bracket Saved
                        </span>
                      </div>
                    ) : (
                      <div className="flex items-center">
                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                          <FaExclamationTriangle className="mr-1" /> No Bracket
                        </span>
                      </div>
                    )}
                  </td>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        ...
                        This will delete their bracket and remove the league from their profile.
                      </p>
                      
                      {kickingUserId.hasBracket && (
                        <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-700">
                          <FaExclamationTriangle className="inline mr-1" />
                          This participant has already filled out their bracket. All their progress will be lost.
                        </div>
                      )}
                    </div>
                  </div>
