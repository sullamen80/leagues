// src/gameTypes/marchMadness/services/bracketService.js
import { collection, doc, getDoc, setDoc, updateDoc, query, where, getDocs } from 'firebase/firestore';
import { db } from '../../../firebase';
import { getDefaultGameData, repairBracket, computeRoundOf64 } from '../utils/bracketUtils';

/**
 * Initialize a new league with March Madness game data
 * @param {string} leagueId - ID of the league to initialize
 * @param {Object} teamsData - Optional teams data from setup
 * @returns {Promise<boolean>} Success status
 */
const initializeLeagueGameData = async (leagueId, teamsData = null) => {
  try {
    if (!leagueId) {
    ...
    // If teams data was provided, use it instead of the default
    if (teamsData) {
      console.log("Using provided teams data", teamsData);
      gameData.SetTeams = teamsData;
    }

    // Generate empty Round of 64 from the team data
    gameData.RoundOf64 = computeRoundOf64(
      gameData.SetTeams.eastRegion,
      gameData.SetTeams.westRegion,
      gameData.SetTeams.midwestRegion,
      gameData.SetTeams.southRegion
    );
    ...
    const defaultLocks = {
      RoundOf64: { locked: false, lockedAt: null },
      RoundOf32: { locked: false, lockedAt: null },
      Sweet16: { locked: false, lockedAt: null },
      Elite8: { locked: false, lockedAt: null },
      FinalFour: { locked: false, lockedAt: null },
      Championship: { locked: false, lockedAt: null }
    };
    
    await setDoc(doc(db, "leagues", leagueId, "locks", "lockStatus"), defaultLocks);

    console.log("March Madness game data initialized for league:", leagueId);
    return true;
  } catch (error) {
    console.error("Error initializing March Madness game data:", error);
    throw error;
  }
};
 ...
 * @param {string} leagueId - League ID
 * @param {Object} tournamentData - Tournament data to use as a base
 * @returns {Promise<boolean>} Success status
 */
...
const createUserBracketFromTemplate = async (leagueId, userId) => {
  try {
    ...
    if (templateSnap.exists()) {
      ...
      console.log("No bracket template found, creating from tournament data");
      
      // Get tournament data instead
      const tournamentRef = doc(db, "leagues", leagueId, "gameData", "current");
      const tournamentSnap = await getDoc(tournamentRef);
      
      if (!tournamentSnap.exists()) {
        throw new Error("No template or tournament data available");
      }
      
      const tournamentData = tournamentSnap.data();
      
      // Create template from tournament data
      await createBracketTemplate(leagueId, tournamentData);
